...
@property (nonatomic,strong) CHCPGQuatationHomeViewController *iHomeVC;
...
- (void)initialization
{
  ...
  _titleLabelBackgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"lunbobiaoti"]];
  ...
  _iHomeVC=[[CHCPGQuatationHomeViewController alloc]init];
  _pageControlStyle = SDCycleScrollViewPageContolStyleAnimated;
  _placeholderImage = [UIImage imageNamed:@"zhuge_default"];

 UIImage *patternImage= [self compressImage:[UIImage imageNamed:@"zhuge_default"]];
...
{
  CGSize size = {self.frame.size.width, self.frame.size.height};
...
- (void)setInfiniteLoop:(BOOL)infiniteLoop
{
  ...
  if (self.imageURLStringsGroup.count) {
    self.imageURLStringsGroup = self.imageURLStringsGroup;
  }
}
...
- (void)layoutSubviews
{
  ...
  CGFloat y = self.mainView.sd_height - size.height - 6;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  int indexOnPageControl = itemIndex % self.imagesGroup.count;
   self.contentOffsetX=self.iHomeVC.iGlobalScrollView.contentOffset.x;
  if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
    TAPageControl *pageControl = (TAPageControl *)_pageControl;
...
//  int count=self.imageURLStringsGroup.count-1;
//  if ((scrollView.contentOffset.x>count *scrollView.frame.size.width)&&translatePoint.x<0) {
//    self.mainView.scrollEnabled = NO;
//    self.isScrolled=NO;
//      self.mainView.allowsSelection = NO;
//  }else if(itemIndex==0&&translatePoint.x>0){
//    self.isScrolled=NO;
//    self.mainView.scrollEnabled=NO;
//    self.mainView.allowsSelection = NO;
//      self.mainView.allowsSelection = NO;
...
//  }
//  if (itemIndex==count&&translatePoint.x<0) {
//    self.mainView.scrollEnabled=NO;
//    self.mainView.allowsSelection=YES;
//  }
//  if (itemIndex==0&&translatePoint.x>0) {
//    self.mainView.scrollEnabled=NO;
//    self.mainView.allowsSelection=NO;
//  }
//  if (itemIndex==0&&translatePoint.x<0) {
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  int scrollSizeWidth=(int)scrollView.frame.size.width;
  int itemIndex = (scrollView.contentOffset.x + self.mainView.sd_width * 0.5) / self.mainView.sd_width;
  UIPanGestureRecognizer *pan=scrollView.panGestureRecognizer;
  CGPoint translatePoint=[pan translationInView:self];
//  if ((itemIndex==self.imageURLStringsGroup.count-1&&translatePoint.x<0)) {
//    self.mainView.scrollEnabled = NO;
//    self.isScrolled=NO;
//  }else if(itemIndex==0&&translatePoint.x>0){
//    self.isScrolled=NO;
...
//    int itemIndex = (scrollView.contentOffset.x + self.mainView.sd_width * 0.5) / self.mainView.sd_width;
////  if ((itemIndex==self.imageURLStringsGroup.count-1)&&(scrollView.contentOffset.x>(self.imageURLStringsGroup.count-1)*scrollView.frame.size.width)) {
//////    self.mainView.scrollEnabled = NO;
...
//   int itemIndex = (scrollView.contentOffset.x ) / self.mainView.sd_width;
//  UIPanGestureRecognizer *pan=scrollView.panGestureRecognizer;
//  CGPoint translatePoint=[pan translationInView:self];
//  int count=self.imageURLStringsGroup.count-1;
//  if ((scrollView.contentOffset.x>count *scrollView.frame.size.width)&&translatePoint.x<0) {
