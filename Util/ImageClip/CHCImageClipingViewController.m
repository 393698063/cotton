...
static const CGFloat kMaxUIImageSize = 640;//图片最大大小
...
typedef enum
{
  EHCClipingShadeDefault = 0,
  EHCClipingShadeS = EHCClipingShadeDefault,
  EHCClipingShadeL,
  EHCClipingShadeP
}EHCClipingShade;
@interface CHCImageClipingViewController ()
...
@property (nonatomic,retain) IBOutlet UIView<CHCImageClipFrame> *frameView;
@property (nonatomic, assign) EHCClipingShade iShadeType;
...
@implementation CHCImageClipingViewController
...
{
  self = [super initWithNibName:@"CHCImageClipingViewController" bundle:nil];
  if(self) {
    ...
    self.iShadeType = EHCClipingShadeP;
    [self selectSLPShadeAction:nil];
    self.minimumScale = 0.2;
    self.maximumScale = 10;
    
    self.doneCallback = aCompletion;
  }
  return self;
}
...
- (void)updateCropRect
{
  self.cropRect = CGRectMake((HC_ImageClip_ScreenBound.size.width-self.cropSize.width)/2,
                             (HC_ImageClip_ScreenBound.size.height-self.cropSize.height)/2,
                    ...
                    imageViewSize:(CGSize)imageViewSize
{
  ...
  if (sourceSize.width>kMaxUIImageSize || sourceSize.height>kMaxUIImageSize)
  {
    scaleSize.width = kMaxUIImageSize;
    scaleSize.height = (sourceSize.height*kMaxUIImageSize)/sourceSize.width;
  }
  else
  {
    scaleSize.width = sourceSize.width;
    scaleSize.height = sourceSize.height;
  }
...
- (IBAction)selectSLPShadeAction:(UIButton *)sender
{
  switch (self.iShadeType)
  {
    case EHCClipingShadeS:
    {
      self.iShadeType = EHCClipingShadeL;
      [self setLandscapeAction:nil];
      break;
    }
    case EHCClipingShadeL:
    {
      self.iShadeType = EHCClipingShadeP;
      [self setLPortraitAction:nil];
      break;
    }
    case EHCClipingShadeP:
    {
      self.iShadeType = EHCClipingShadeS;
      [self setSquareAction:nil];
      break;
    }
    default:
    {
      self.iShadeType=self.iShadeType+1%3;
      break;
    }
  }
}
