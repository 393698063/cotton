...
@implementation CHCPopupMenuView
...
{
  CGFloat arrowsize =  [CHCPopupMenu sharedMenu].iArrowSize;
  if (!arrowsize)
  {
    arrowsize = ARROWSIZE;
  }
             ...
             menuItems:(NSArray *)menuItems
{
  ...
  UIImage *aimage = [CHCPopupMenu sharedMenu].iBgImage;
  ...
  CGRect userShadeFrame = [CHCPopupMenu sharedMenu].iShaDeFrame;
  //如果使用者更改遮罩的frame
  if (userShadeFrame.size.width>0)
  {
    aShadeFrame = userShadeFrame;
  }
  CHCPopMenuBaseView *overlay = [[CHCPopMenuBaseView alloc] initWithFrame:aShadeFrame];
...
- (UIView *) mkContentView
{
  ...
  UIFont *titleFont = [CHCPopupMenu titleFont];
  ...
  for (CHCPopupMenuItem *menuItem in iMenuItems)
  {
  ...
  UIImage *selectedImage = [CHCPopupMenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight + 2}];
  if (aMenu.iMenuItemSelectBgImage)
  {
    selectedImage = aMenu.iMenuItemSelectBgImage;
  }
  //分割线
  UIImage *gradientLine = [CHCPopupMenuView gradientLine: (CGSize){maxItemWidth - kMarginX * 4, 1}];
...

{
  ...
  self.iContentView = [self mkContentViewkMinMenuItemHeight:kMinMenuItemWidth
                                       ...
                                       kMarginY            :kMarginY];
  //弹出菜单的frame
  CGRect   aframe = CGRectMake(192, 32, 128, 96);
  //ios7适配 弹出框y坐标下移20px
  if (HC_CURENT_OS_V >= 7.0f)
  {
    aframe = CGRectMake(192, 48, 128, 96);
  }
  iBaseView = [[CHCPopMenuBaseView alloc]initWithFrame:view.bounds];
  UIImageView *bgView = [[UIImageView alloc]initWithFrame:aframe];
  NSString *bgimageName = [CHCPopupMenu sharedMenu].iMenuBgImageName;
  if (!bgimageName)
  {
    bgimageName = @"tanchu_beijing";
  }
  
  bgView.image = [UIImage imageNamed:bgimageName];
  [self.iBaseView addSubview:bgView];
  
  CGRect contentViewFrame = CGRectMake(aframe.origin.x+12, aframe.origin.y+16,
                                       self.iContentView.frame.size.width+12, self.iContentView.frame.size.height+16);
...
- (void)performActionOld:(id)sender
{
  ...
  CHCPopupMenuItem    *menuItem = iMenuItems[button.tag];
    ...
    if (animated)
    {
      ...
      [UIView animateWithDuration:0.2
                       ...
                       completion:^(BOOL finished)
       {
         ...
         if ([self.superview isKindOfClass:[CHCPopMenuBaseView class]])
...

{
  ...
  for (CHCPopupMenuItem *menuItem in iMenuItems)
  {
    ...
    if (menuItem.enabled)
    {
      ...
      button.opaque = NO;
      button.autoresizingMask = UIViewAutoresizingNone;
      [button setTitle:menuItem.iTitle forState:UIControlStateNormal];
      UIColor *titleColorNorm = menuItem.iFforeColor ? menuItem.iFforeColor :[UIColor whiteColor];
      [button setTitleColor:titleColorNorm forState:UIControlStateNormal];
      UIColor *titleColorHighLight = menuItem.iHighLightColor ? menuItem.iHighLightColor :[UIColor whiteColor];
      [button setTitleColor:titleColorHighLight forState:UIControlStateHighlighted];
      button.titleLabel.font = titleFont;
      if(menuItem.iImage)
      {
        [button setContentHorizontalAlignment:UIControlContentHorizontalAlignmentLeft];
        [button setImage:menuItem.iImage forState:UIControlStateNormal];
        [button setImage:menuItem.iHighLightImage forState:UIControlStateHighlighted];
      }
      else
      {
    ...
    //为了实现点击menuItem高亮显示文字和图片，我们对这部分逻辑做了一些修改，原来是在button上加了一个imageview和一个label分别放图片和title，这种实现没有办法实现高亮，并且实现逻辑层次太多，不合理，修改后直接给button添加图片和title，分别设置normal和highLight的图片和文字颜色来实现点击后高亮显示
...
//      //modify by jimh 由于menuItem.iAalignment默认值为NSTextAlignmentLeft，并且menuItem.iAalignment变量是整形的，用上述语句起不到判断作用，改成下面的语句，取消默认居中的设置，使用者需手工设置对齐方式
//      titleLabel.textAlignment = menuItem.iAalignment;
//
//      titleLabel.textColor = menuItem.iFforeColor ? menuItem.iFforeColor :[UIColor whiteColor];
//
             ...
             inContext:(CGContextRef) context
{
  ...
  CGFloat alpha = 1;
  //有背景图片，不设置渐变！
  if ([CHCPopupMenu sharedMenu].iBgImage)
  {
    alpha = 0;
  }
  ...
  UIColor *tintColor = [CHCPopupMenu tintColor];
  if (tintColor)
  {
